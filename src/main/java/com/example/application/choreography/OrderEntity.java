...
public class OrderEntity extends EventSourcedEntity<Order, OrderEvent> {
  ...
  public Effect<Response> placeOrder(@RequestBody PlaceOrder placeOrder) {
    OrderPlaced orderPlaced = new OrderPlaced(commandContext().entityId(), placeOrder.userId(), placeOrder.productId(), placeOrder.quantity(), placeOrder.price());
    return effects()
  ...
  @PatchMapping("/reject")
  public Effect<Response> rejectOrder() {
    return effects()
      .emitEvent(new OrderRejected(commandContext().entityId()))
      .thenReply(__ -> Success.of("order placed"));
  }

  @PatchMapping("/confirm")
  public Effect<Response> confirmOrder() {
    return effects()
      .emitEvent(new OrderConfirmed(commandContext().entityId()))
      .thenReply(__ -> Success.of("order placed"));
  }
